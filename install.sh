...
install_docker_ntfy() {
  ...
  docker run -v /var/cache/ntfy:/var/cache/ntfy -v /etc/ntfy:/etc/ntfy -v /etc/letsencrypt:/etc/letsencrypt -p $port:80 -itd binwiederhier/ntfy serve --cache-file /var/cache/ntfy/cache.db
